...
//		}
	...
	public static List<DepartmentDetail> listDepartmentsDetails(String accessToken, List<Department> lsDepart)
			throws  ServiceNotExistException, SdkInitException, ServiceException {
		CorpDepartmentService corpDepartmentService = ServiceFactory.getInstance().getOpenService(CorpDepartmentService.class);
		List<DepartmentDetail> dd =new ArrayList();
		
		for(int i=0;i<lsDepart.size();i++){
			//System.out.println(lsDepart.get(i).getId().toString());
			dd.add((DepartmentDetail) corpDepartmentService.getDeptDetail(accessToken,lsDepart.get(i).getId().toString()));
		}
		return dd;
	}
	//public static String testDepartment()
